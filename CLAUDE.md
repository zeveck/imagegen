# openai-imagegen -- Agent Reference

## Architecture

A Claude Code skill that generates images via the ChatGPT/OpenAI image generation API.

```
.claude/skills/   — Claude Code skill definitions (zskills + project skills)
.claude/hooks/    — Safety hooks and session logging
scripts/          — Helper scripts (port, test runner, briefing)
src/              — Skill source code and utilities
tests/            — Test files
```
...
```bash
# No dev server — this is a CLI skill, not a web app
```

The port is determined automatically by `scripts/port.js`: **8080** for the main repo (`/workspaces/openai-imagegen`), a **deterministic unique port** for each worktree (derived from the project root path). Run `node scripts/port.js` to see your port. Override with `DEV_PORT=NNNN` env var if needed.
...
**Auth gate:** N/A — no auth gate for this project.

## Tests

```bash
npm test             # Unit tests only -- fast, use while working
npm run test:all     # ALL suites -- use before committing
```

**`npm run test:all` must pass before every commit.** When reporting test
...
**NEVER modify the working tree to check if a failure is pre-existing.** No `git stash && npm test && git stash pop`, no `git checkout <old-commit>`, no temporary worktrees for comparison. These workflows are fragile -- context compaction between the modification and the restore will lose your changes. Past failure: an agent stashed changes, checked out a prior commit to verify a test failure was pre-existing, hit compaction, and never restored the working tree. If you touched code and tests fail, fix them. If you only touched content (markdown, images, etc.), don't run tests at all.
...
`npm run test:all > test-output.log 2>&1` -- then read the file to
...
- `tests/**/*.test.ts` — unit tests (TypeScript)
